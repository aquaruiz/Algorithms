...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			makeCombWithoutRepetition(arr, 0, maxNum, 1);
	...
	private static void makeCombWithoutRepetition(int[] arr, int num, int max, int border) {
		if (num == arr.length) {
			System.out.println(
						Arrays.stream(arr)
		...
		for (int i = border; i <= max; i++) {
			arr[num] = i;
			makeCombWithoutRepetition(arr, num + 1, max, i + 1); 
		}
	}
